Desc:Go sync 包的使用方法，`sync.Mutex` ，`sync.RMutex`，`sync.Once` ， `sync.Cond`，`sync.Waitgroup` 尽管 Golang 推荐通过 `channel` 进行通信和同步，但在实际开发中 `sync`包用得也非常的多。另外 `sync` 下还有一个 `atomic` 包，提供了一些底层的原子操作（这里不做介绍）。本篇文章主要介绍该包下的锁的一些概念及使用方法。
